...
void atc_init(atc_t *atc, const char *name, USART_TypeDef *USARTx, void (*found)(char*))
{
  ...
  atc->usart = USARTx;
  atc->found = found;
  /*LL_USART_EnableIT_RXNE*/LL_USART_EnableIT_RXNE_RXFNE(atc->usart);
  atc->inited = true;
  atc_printf("\r\n[%s] inited.\r\n", atc->name);
}
//####################################################################################################
...
void atc_transmit(atc_t *atc, uint8_t *data, uint16_t len)
{
  ...
  {
    while (!/*LL_USART_IsActiveFlag_TXE*/LL_USART_IsActiveFlag_TXE_TXFNF(atc->usart))
      atc_delay(1);
...
{
  if (/*LL_USART_IsActiveFlag_RXNE*/LL_USART_IsActiveFlag_RXNE_RXFNE(atc->usart))
  {
    ...
    int items, ...)
{
  ...
  atc_printf("[%s] %s", atc->name, command, (char* )atc->rxBuffer);
  uint32_t start = HAL_GetTick();
